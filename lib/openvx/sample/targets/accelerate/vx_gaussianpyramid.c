...
#include <VX/vx_lib_debug.h> // still use this! for definition of debug kernels
...
{
    vx_context context = vxGetContext((vx_reference)node);
    return VX_SUCCESS; // DONT'T ACTUALLY NEED: vxLoadKernels(context, "openvx-debug");
}
...
vx_kernel_description_t gaussian_pyramid_kernel = {
    ...
    "com.machineswithvision.openvx.pyramid",
    vxPyramidKernel,
