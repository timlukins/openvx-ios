...
// TODO: could Use special clang extension to define a vector or
...
static vx_status VX_CALLBACK vxAndKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    if (num == 3)
    {
        ...
        return vxAnd(in1, in2, output); // TODO:
    }
    return VX_ERROR_INVALID_PARAMETERS;
}

static vx_status VX_CALLBACK vxAndInputValidator(vx_node node, vx_uint32 index)
{
...
static vx_status VX_CALLBACK vxAndOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 2)
    {
...
static vx_param_description_t and_kernel_params[] = {
...
vx_kernel_description_t and_kernel = {
    VX_KERNEL_AND,
    "com.machineswithvision.openvx.and",
    vxAndKernel,
