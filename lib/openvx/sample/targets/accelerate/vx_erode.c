...
vx_status vxErode3x3(vx_image src, vx_image dst, vx_border_mode_t *borders)
{
    ...
    */
    ...
    result = vImageErode_Planar8(&srcVimg, &dstVimg, 0, 0, kernel, 3, 3, kvImageNoFlags);
...
static vx_status VX_CALLBACK vxErodeInputValidator(vx_node node, vx_uint32 index)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 0)
    {
...
static vx_status VX_CALLBACK vxErodeOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 1)
    {
...
static vx_param_description_t erode_kernel_params[] = {
...
vx_kernel_description_t erode3x3_kernel = {
    VX_KERNEL_ERODE_3x3,
    "com.machineswithvision.openvx.erode3x3",
    vxErode3x3Kernel,
