...
static vx_status VX_CALLBACK vxOrInputValidator(vx_node node, vx_uint32 index)
{
...
static vx_status VX_CALLBACK vxOrOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 2)
    {
...
static vx_param_description_t or_kernel_params[] = {
...
vx_kernel_description_t or_kernel = {
    VX_KERNEL_OR,
    "com.machineswithvision.openvx.or",
    vxOrKernel,
    or_kernel_params, dimof(or_kernel_params),
    vxOrInputValidator,
    vxOrOutputValidator,
    NULL,
    NULL,
